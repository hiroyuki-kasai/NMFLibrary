function [x, infos] = vb_pro_nmf(V, rank, in_options)
...
%
    ...
    % initialize
    if options.ard
        method_name = 'VB-Pro-NMF (ARD)';
    else
        method_name = 'VB-Pro-NMF';
    end
    epoch = 0;    
    grad_calc_count = 0;
    ...
    % store initial info
    clear infos;
    [infos, f_val, optgap] = store_nmf_info(V, exp_U, exp_V', [], options, [], epoch, grad_calc_count, 0);
    
    if options.verbose > 1
        if options.ard
            fprintf('VB-Pro-NMF (ARD): Epoch = 0000, cost = %.16e, optgap = %.4e\n', f_val, optgap); 
        else
            fprintf('VB-Pro-NMF: Epoch = 0000, cost = %.16e, optgap = %.4e\n', f_val, optgap); 
        end
    ...
    while true
        ...
        % store info
        infos = store_nmf_info(V, exp_U, exp_V', [], options, infos, epoch, grad_calc_count, elapsed_time);       
